...
/**
 * Service for building credibility timeline chart render arrays.
 * 
 * Specialized service for credibility score trends that filters news source data
 * to show only credibility metrics over time. Follows the same design pattern as
 * the combined NewsSourceTimelineChartService.
 */
class CredibilityTimelineChartService {
  ...
  /**
   * Build credibility timeline chart render array.
   *
   * @param array $options
   *   Chart configuration options.
   *
   * @return array
   *   Render array for credibility timeline chart.
   */
  public function buildCredibilityTimelineChart(array $options = []): array {
    // Set default options
    $options = $options + [
      'limit' => 10,
      'days_back' => 90,
    ...
    // Filter data to only include credibility metrics
    $chart_data['timeline_data'] = $this->filterCredibilityData($chart_data['timeline_data']);
    if (!empty($extended_sources['timeline_data'])) {
      $extended_sources['timeline_data'] = $this->filterCredibilityData($extended_sources['timeline_data']);
    }
    ...
    if ($options['show_title'] && !empty($options['title'])) {
      $build['header'] = [
        ...
        'description' => [
          '#type' => 'html_tag',
          '#tag' => 'p',
          '#value' => 'This chart shows credibility scores for news sources over the past ' . $options['days_back'] . ' days. Credibility scale: 0 = Active deception/misinformation, 100 = Only provable facts.',
    ...
    $build['#attached']['drupalSettings']['newsmotivationmetrics_credibility'] = $this->buildJavaScriptSettings($chart_data, $options, $extended_sources);
  ...
  /**
   * Build credibility timeline block for use in Drupal blocks.
   *
   * @param array $config
   *   Block configuration.
   *
   * @return array
   *   Render array for credibility timeline block.
   */
  public function buildCredibilityTimelineBlock(array $config = []): array {
    // Convert block config to chart options
    $options = [
      'canvas_id' => 'credibility-timeline-chart-' . substr(md5(serialize($config)), 0, 8),
      'title' => $config['chart_title'] ?? 'News Source Credibility Trends',
      ...
      'container_classes' => ['credibility-timeline-block'],
      'library' => 'newsmotivationmetrics/credibility-timeline',
      'js_behavior' => 'credibilityTimelineChart',
    ];

    return $this->buildCredibilityTimelineChart($options);
  }

  /**
   * Filter timeline data to only include credibility metrics.
   *
   * @param array $timeline_data
   *   Full timeline data from service.
   *
   * @return array
   *   Filtered timeline data containing only credibility metrics.
   */
  ...
  protected function buildChartControls(array $extended_sources_data, string $canvas_id): array {
    ...
    $controls['controls_container']['selector_group']['label'] = [
      ...
      '#attributes' => [
        'for' => $canvas_id === 'credibility-timeline-chart' ? 'source-selector' : 'source-selector-' . substr($canvas_id, -8),
        'class' => ['control-label'],
      ],
    ];
    ...
    $controls['controls_container']['selector_group']['selector'] = [
      ...
      '#attributes' => [
        'id' => $canvas_id === 'credibility-timeline-chart' ? 'source-selector' : 'source-selector-' . substr($canvas_id, -8),
    ...
    $controls['controls_container']['help_text'] = [
      '#type' => 'html_tag',
      '#tag' => 'div',
      '#value' => '💡 <strong>Credibility Score Scale:</strong> 0 = Active deception/misinformation, 50 = Mixed accuracy, 100 = Only provable facts. Use Ctrl+Click to select multiple sources.',
    ...
    return [
      ...
      'canvas' => [
        ...
        '#attributes' => [
          ...
          'class' => ['credibility-timeline-chart'],
          'style' => 'max-width: 100%; height: ' . $options['chart_height'] . 'px; max-height: 500px;',
          'aria-label' => 'News Source Credibility Trends - Interactive Timeline Chart',
        ],
      ],
    ];
  }
  ...
  protected function buildEmptyChart(array $options): array {
    return [
      ...
      'message' => [
        '#type' => 'html_tag',
        '#tag' => 'div',
        '#value' => '📊 No credibility data available for the selected time period.',
  ...
  protected function buildJavaScriptSettings(array $chart_data, array $options, array $extended_sources = null): array {
    ...
    if ($options['js_behavior'] === 'credibilityTimelineBlocks') {
      return [
        'blocks' => [
