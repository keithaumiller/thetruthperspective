...
/**
 * Service for building bias timeline chart render arrays.
 * 
 * Specialized service for bias rating trends that filters news source data
 * to show only bias metrics over time. Follows the same design pattern as
 * the combined NewsSourceTimelineChartService.
 */
class BiasTimelineChartService {
  ...
  /**
   * Build bias timeline chart render array.
   *
   * @param array $options
   *   Chart configuration options.
   *
   * @return array
   *   Render array for bias timeline chart.
   */
  public function buildBiasTimelineChart(array $options = []): array {
    // Set default options
    $options = $options + [
      'limit' => 10,
      'days_back' => 90,
    ...
    // Filter data to only include bias metrics
    $chart_data['timeline_data'] = $this->filterBiasData($chart_data['timeline_data']);
    if (!empty($extended_sources['timeline_data'])) {
      $extended_sources['timeline_data'] = $this->filterBiasData($extended_sources['timeline_data']);
    }
    ...
    if ($options['show_title'] && !empty($options['title'])) {
      $build['header'] = [
        ...
        'description' => [
          '#type' => 'html_tag',
          '#tag' => 'p',
          '#value' => 'This chart shows bias ratings for news sources over the past ' . $options['days_back'] . ' days. Bias rating scale: 0 = Left-leaning, 50 = Center, 100 = Right-leaning.',
    ...
    $build['#attached']['drupalSettings']['newsmotivationmetrics_bias'] = $this->buildJavaScriptSettings($chart_data, $options, $extended_sources);
  ...
  /**
   * Build bias timeline block for use in Drupal blocks.
   *
   * @param array $config
   *   Block configuration.
   *
   * @return array
   *   Render array for bias timeline block.
   */
  public function buildBiasTimelineBlock(array $config = []): array {
    // Convert block config to chart options
    $options = [
      'canvas_id' => 'bias-timeline-chart-' . substr(md5(serialize($config)), 0, 8),
      'title' => $config['chart_title'] ?? 'News Source Bias Trends',
      ...
      'container_classes' => ['bias-timeline-block'],
      'library' => 'newsmotivationmetrics/bias-timeline',
      'js_behavior' => 'biasTimelineChart',
    ];

    return $this->buildBiasTimelineChart($options);
  }

  /**
   * Filter timeline data to only include bias metrics.
   *
   * @param array $timeline_data
   *   Full timeline data from service.
   *
   * @return array
   *   Filtered timeline data containing only bias metrics.
   */
  ...
  protected function buildChartControls(array $extended_sources_data, string $canvas_id): array {
    ...
    $controls['controls_container']['selector_group']['label'] = [
      ...
      '#attributes' => [
        'for' => $canvas_id === 'bias-timeline-chart' ? 'source-selector' : 'source-selector-' . substr($canvas_id, -8),
        'class' => ['control-label'],
      ],
    ];
    ...
    $controls['controls_container']['selector_group']['selector'] = [
      ...
      '#attributes' => [
        'id' => $canvas_id === 'bias-timeline-chart' ? 'source-selector' : 'source-selector-' . substr($canvas_id, -8),
    ...
    $controls['controls_container']['help_text'] = [
      '#type' => 'html_tag',
      '#tag' => 'div',
      '#value' => '💡 <strong>Bias Rating Scale:</strong> 0 = Left-leaning, 50 = Center/Neutral, 100 = Right-leaning. Use Ctrl+Click to select multiple sources.',
    ...
    return [
      ...
      'canvas' => [
        ...
        '#attributes' => [
          ...
          'class' => ['bias-timeline-chart'],
          'style' => 'max-width: 100%; height: ' . $options['chart_height'] . 'px; max-height: 500px;',
          'aria-label' => 'News Source Bias Trends - Interactive Timeline Chart',
        ],
      ],
    ];
  }
  ...
  protected function buildEmptyChart(array $options): array {
    return [
      ...
      'message' => [
        '#type' => 'html_tag',
        '#tag' => 'div',
        '#value' => '📊 No bias data available for the selected time period.',
  ...
  protected function buildJavaScriptSettings(array $chart_data, array $options, array $extended_sources = null): array {
    ...
    if ($options['js_behavior'] === 'biasTimelineBlocks') {
      return [
        'blocks' => [
