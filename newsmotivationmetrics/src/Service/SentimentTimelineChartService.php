...
/**
 * Service for building sentiment timeline chart render arrays.
 * 
 * Specialized service for sentiment score trends that filters news source data
 * to show only sentiment metrics over time. Follows the same design pattern as
 * the combined NewsSourceTimelineChartService.
 */
class SentimentTimelineChartService {
  ...
  /**
   * Build sentiment timeline chart render array.
   *
   * @param array $options
   *   Chart configuration options.
   *
   * @return array
   *   Render array for sentiment timeline chart.
   */
  public function buildSentimentTimelineChart(array $options = []): array {
    // Set default options
    $options = $options + [
      'limit' => 10,
      'days_back' => 90,
    ...
    // Filter data to only include sentiment metrics
    $chart_data['timeline_data'] = $this->filterSentimentData($chart_data['timeline_data']);
    if (!empty($extended_sources['timeline_data'])) {
      $extended_sources['timeline_data'] = $this->filterSentimentData($extended_sources['timeline_data']);
    }
    ...
    if ($options['show_title'] && !empty($options['title'])) {
      $build['header'] = [
        ...
        'description' => [
          '#type' => 'html_tag',
          '#tag' => 'p',
          '#value' => 'This chart shows sentiment scores for news sources over the past ' . $options['days_back'] . ' days. Sentiment scale: 0 = Negative emotional tone, 50 = Neutral, 100 = Positive emotional tone.',
    ...
    $build['#attached']['drupalSettings']['newsmotivationmetrics_sentiment'] = $this->buildJavaScriptSettings($chart_data, $options, $extended_sources);
  ...
  /**
   * Build sentiment timeline block for use in Drupal blocks.
   *
   * @param array $config
   *   Block configuration.
   *
   * @return array
   *   Render array for sentiment timeline block.
   */
  public function buildSentimentTimelineBlock(array $config = []): array {
    // Convert block config to chart options
    $options = [
      'canvas_id' => 'sentiment-timeline-chart-' . substr(md5(serialize($config)), 0, 8),
      'title' => $config['chart_title'] ?? 'News Source Sentiment Trends',
      ...
      'container_classes' => ['sentiment-timeline-block'],
      'library' => 'newsmotivationmetrics/sentiment-timeline',
      'js_behavior' => 'sentimentTimelineChart',
    ];

    return $this->buildSentimentTimelineChart($options);
  }

  /**
   * Filter timeline data to only include sentiment metrics.
   *
   * @param array $timeline_data
   *   Full timeline data from service.
   *
   * @return array
   *   Filtered timeline data containing only sentiment metrics.
   */
  ...
  protected function buildChartControls(array $extended_sources_data, string $canvas_id): array {
    ...
    $controls['controls_container']['selector_group']['label'] = [
      ...
      '#attributes' => [
        'for' => $canvas_id === 'sentiment-timeline-chart' ? 'source-selector' : 'source-selector-' . substr($canvas_id, -8),
        'class' => ['control-label'],
      ],
    ];
    ...
    $controls['controls_container']['selector_group']['selector'] = [
      ...
      '#attributes' => [
        'id' => $canvas_id === 'sentiment-timeline-chart' ? 'source-selector' : 'source-selector-' . substr($canvas_id, -8),
    ...
    $controls['controls_container']['help_text'] = [
      '#type' => 'html_tag',
      '#tag' => 'div',
      '#value' => '💡 <strong>Sentiment Score Scale:</strong> 0 = Negative emotional tone, 50 = Neutral tone, 100 = Positive emotional tone. Use Ctrl+Click to select multiple sources.',
    ...
    return [
      ...
      'canvas' => [
        ...
        '#attributes' => [
          ...
          'class' => ['sentiment-timeline-chart'],
          'style' => 'max-width: 100%; height: ' . $options['chart_height'] . 'px; max-height: 500px;',
          'aria-label' => 'News Source Sentiment Trends - Interactive Timeline Chart',
        ],
      ],
    ];
  }
  ...
  protected function buildEmptyChart(array $options): array {
    return [
      ...
      'message' => [
        '#type' => 'html_tag',
        '#tag' => 'div',
        '#value' => '📊 No sentiment data available for the selected time period.',
  ...
  protected function buildJavaScriptSettings(array $chart_data, array $options, array $extended_sources = null): array {
    ...
    if ($options['js_behavior'] === 'sentimentTimelineBlocks') {
      return [
        'blocks' => [
