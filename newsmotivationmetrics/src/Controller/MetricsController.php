...
class MetricsController extends ControllerBase {
   ...
   * 
   * Provides clean environment for testing Chart.js integration
   * using proper Drupal render arrays and separate CSS/JS files.
   * 
   * @return array
   *   Drupal render array for debug page.
   */
  public function chartDebug() {
    ...
    $build['header']['title'] = [
      '#type' => 'html_tag',
      '#tag' => 'h1',
      '#value' => '📊 Chart Debug Page',
    ];
    
    $build['header']['description'] = [
      '#type' => 'html_tag',
      '#tag' => 'p',
      '#value' => 'Testing Chart.js integration for The Truth Perspective Analytics',
    ];
    ...
    $build['debug_info']['stats'] = [
      '#type' => 'markup',
      '#markup' => '<div>Data Points: ' . count($timeline_data) . '</div>
    ...
    $build['data_preview']['content'] = [
      '#type' => 'html_tag',
      '#tag' => 'pre',
      '#value' => htmlspecialchars(json_encode($timeline_data, JSON_PRETTY_PRINT)),
